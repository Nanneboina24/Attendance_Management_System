...
public class DBAdapter extends SQLiteOpenHelper {
	...
	public StudentBean validateStudent(String userName,String password)
	{
		...
		String query = "SELECT * FROM student_table where student_firstname='"+userName+"' and student_mobilenumber='"+password+"'";
		Cursor cursor = db.rawQuery(query, null);
	...
	public int addAttendanceSession(AttendanceSessionBean attendanceSessionBean) {
		...
		String query = "INSERT INTO attendance_session_table (attendance_session_faculty_id,attendance_session_department,attendance_session_class,attendance_session_sem,attendance_session_date,attendance_session_subject) values ('"+
				...
				attendanceSessionBean.getAttendance_session_sem()+"', '"+
	...
	public ArrayList<AttendanceBean> getAttendanceBySessionID(AttendanceSessionBean attendanceSessionBean)
	{
		...
		if(cursor1.moveToFirst()) 
		{
			...
			AttendanceBean attendanceBeans2 = new AttendanceBean();
			attendanceBeans2.setAttendance_session_id(-2);
			attendanceBeans2.setAttendance_student_id(Integer.parseInt(totalcount(attendanceSessionBean)));
			attendanceBeans2.setAttendance_status("Total   Count : "+Integer.parseInt(totalcount(attendanceSessionBean)));
			list.add(attendanceBeans2);

			AttendanceBean attendanceBeans3 = new AttendanceBean();
			attendanceBeans3.setAttendance_session_id(-3);
			attendanceBeans3.setAttendance_student_id(Integer.parseInt(presentcount(attendanceSessionId)));
			attendanceBeans3.setAttendance_status("Present Count : "+Integer.parseInt(presentcount(attendanceSessionId)));
			list.add(attendanceBeans3);
	...
	public ArrayList<AttendanceBean> getTotalAttendanceBySessionID(AttendanceSessionBean attendanceSessionBean)
	{
		...
		if(cursor.moveToFirst()) 
		{
			do{
				...
				attendanceBean.setAttendance_status("Date : " + cursor.getString(5));
				list.add(attendanceBean);
				String dat2=cursor.getString(5);
				...
				AttendanceBean attendanceBeans = new AttendanceBean();
				attendanceBeans.setAttendance_session_id(-1);
				attendanceBeans.setAttendance_status("Session : " +session);
				list.add(attendanceBeans);
				dat1=cursor.getString(5);
	...
	public ArrayList<AttendanceBean> getAnyAttendance(String branch,String year,String subject,String ymd)
	{
      ...
      ArrayList<AttendanceBean> list=new ArrayList<AttendanceBean>();
      SQLiteDatabase db = this.getWritableDatabase();
		/*String query = "SELECT * FROM attendance_session_table where attendance_session_date="+ymd+""
				+" AND attendance_session_department="+branch+" AND attendance_session_class="+year+" " +
				" AND attendance_session_subject="+subject+" ";*/
		String query="SELECT * FROM attendance_session_table WHERE attendance_session_date='"+ymd+"' AND attendance_session_department='"+branch+"' AND attendance_session_class='"+year+"' AND attendance_session_subject='"+subject+"' ";
		Log.d("query", query);
		Cursor cursor = db.rawQuery(query, null);
		//System.out.println("DB");
		if(cursor.moveToFirst())
		...
		//	System.out.println("DB1");
			...
			/*	System.out.println("DB2");
                System.out.println(attendanceSessionId);*/
				String query1="SELECT * FROM attendance_table where attendance_session_id=" + attendanceSessionId+" order by attendance_student_id";
				Log.d("query1", query1);
				Cursor cursor1 = db.rawQuery(query1, null);
				if(cursor1.moveToFirst())
				...
				AttendanceBean attendanceBeans2 = new AttendanceBean();
				attendanceBeans2.setAttendance_session_id(-2);
				attendanceBeans2.setAttendance_student_id(Integer.parseInt(totalcount2(branch,year)));
				attendanceBeans2.setAttendance_status("Total   Count : "+Integer.parseInt(totalcount2(branch,year)));
				list.add(attendanceBeans2);
	...
	public ArrayList<AttendanceBean> getAttendanceByBYSCount(String branch,String year,String subject)
	{
		ArrayList<AttendanceBean> list = new ArrayList<AttendanceBean>();
        String str="P";
		SQLiteDatabase db = this.getWritableDatabase();
		String query = "SELECT attendance_student_id,count(*) FROM attendance_table WHERE attendance_session_id in (select attendance_session_id FROM attendance_session_table WHERE attendance_session_department='"+branch+"' AND attendance_session_class='"+year+"' AND attendance_session_subject='"+subject+"') AND attendance_status='"+str+"' group by attendance_student_id ";
		...
		/*select stuid ,count(*) from atten1 where sid in (select sid from atten2 where dept="cse" and class="1Y" and subject="A1")
		and status="P" group by stuid;


		select a1.stuid,count(*) from atten1 a1 inner join atten2 a2 on a1.sid=a2.sid
		where a2.dept="cse" and a2.class="1y" and a2.subject="A1" and a1.status="P" group by a1.stuid;

		select a1.stuid,count(*) from atten1 a1,atten2 a2 where a1.sid=a2.sid
		and a2.dept="cse" and a2.class="1y" and a2.subject="A1" and a1.status="P" group by a1.stuid;*/

	}

	public String totalcount(AttendanceSessionBean attendanceSessionBean)
	{
		String count="";
		SQLiteDatabase db = this.getWritableDatabase();
		String query=" SELECT COUNT(*) FROM student_table WHERE student_department='"+attendanceSessionBean.getAttendance_session_department()+"' AND student_class ='"+attendanceSessionBean.getAttendance_session_class()+"'   ";
		Cursor cursor = db.rawQuery(query, null);
		if(cursor.moveToFirst()) {
	...
	public String totalcount2(String branch,String year)
	{
		String count="";
		SQLiteDatabase db = this.getWritableDatabase();
		String query=" SELECT COUNT(*) FROM student_table WHERE student_department='"+branch+"' AND student_class ='"+year+"'   ";
		Cursor cursor = db.rawQuery(query, null);
		if(cursor.moveToFirst()) {
			count = cursor.getString(0);
		}
	...
	public String presentcount(int attendanceSessionId)
	{
		String count="";
		String str="P";
		SQLiteDatabase db = this.getWritableDatabase();
		String query=" SELECT COUNT(*) FROM attendance_table WHERE attendance_session_id='"+attendanceSessionId+"' AND attendance_status ='"+str+"'   ";
		Cursor cursor = db.rawQuery(query, null);
		if(cursor.moveToFirst()) {
			count = cursor.getString(0);
		}
	...
	public ArrayList<AttendanceBean> getDayStatus(StudentBean studentBean,String dat)
	{
		ArrayList<AttendanceBean> list = new ArrayList<AttendanceBean>();
		SQLiteDatabase db = this.getWritableDatabase();

		String query = "SELECT * FROM attendance_table WHERE attendance_session_id in (select attendance_session_id FROM attendance_session_table WHERE attendance_session_department='"+studentBean.getStudent_department()+"' AND attendance_session_class='"+studentBean.getStudent_class()+"' AND attendance_session_date='"+dat+"') And attendance_student_id="+studentBean.getStudent_id()+" ";
		Log.d("query", query);
	...
	public String getSubject(int sessionid)
	{
		String subject="";
		SQLiteDatabase db = this.getWritableDatabase();

		String query="select attendance_session_subject from attendance_session_table where attendance_session_id="+sessionid+" ";
	...
	/*public ArrayList<AttendanceBean> getSubjectNames(StudentBean studentBean)
	{
		ArrayList<AttendanceBean> list=new ArrayList<AttendanceBean>();
		SQLiteDatabase db = this.getWritableDatabase();
	...
	public ArrayList<AttendanceBean> getSubjectNames(StudentBean studentBean)
	{
		...
		String query = "SELECT DISTINCT(attendance_session_subject) FROM attendance_session_table WHERE attendance_session_department='"+studentBean.getStudent_department()+"' AND attendance_session_class='"+studentBean.getStudent_class()+"' ";
		Log.d("query", query);
		...
		if(cursor.moveToFirst())
		{
			do{
				...
				AttendanceBean attendanceBean = new AttendanceBean();
				attendanceBean.setAttendance_status(cursor.getString(0));
	...
	public String gettotalsessions(String subject)
	{
		String total="";
		SQLiteDatabase db = this.getWritableDatabase();

		String query="SELECT COUNT(*) FROM attendance_session_table WHERE attendance_session_subject='"+subject+"' ";
		Log.d("query", query);
	...
	public String getrangetotalsessions(String branch,String year,String subject,String from,String to)
	{
		String total="";
		SQLiteDatabase db = this.getWritableDatabase();

		String query="SELECT COUNT(*) FROM attendance_session_table WHERE attendance_session_department='"+branch+"' AND attendance_session_class='"+year+"' AND attendance_session_subject='"+subject+"' AND attendance_session_date BETWEEN '"+from+"' AND '"+to+"'";
		Log.d("query", query);
	...
	public String getpresentsessions(int id,String subject)
	{
		String present="";
		String str="P";
		SQLiteDatabase db = this.getWritableDatabase();

		String query="SELECT COUNT(*) FROM attendance_table WHERE attendance_session_id IN (SELECT attendance_session_id FROM attendance_session_table WHERE attendance_session_subject='"+subject+"') AND attendance_student_id="+id+" AND attendance_status='"+str+"'    ";
		...
		if(cursor.moveToFirst()) {
			present = cursor.getString(0);
		}

		return present;
	}

	public String getrangepresentsessions(String branch,String year,String subject,String from,String to,int id)
	{
		String present="";
		String str="P";
		SQLiteDatabase db = this.getWritableDatabase();

		String query="SELECT COUNT(*) FROM attendance_table WHERE attendance_session_id IN (SELECT attendance_session_id FROM attendance_session_table  WHERE attendance_session_department='"+branch+"' AND attendance_session_class='"+year+"' AND attendance_session_subject='"+subject+"' AND attendance_session_date BETWEEN '"+from+"' AND '"+to+"') AND attendance_student_id="+id+" AND attendance_status='"+str+"' ";
